...
Rewrites the python package name, the console-script name, the ``MEETCAP`` env-var prefix, the
Terraform resource-id stem and the distribution / git id across the tree, so a fork does not have
to hunt them down by hand. It is a mechanical text plus directory rename, deliberately
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_meetings --cli acme-meetings \
        --env-prefix ACME --resource acme-meetings --yes

Every rule is applied in ONE simultaneous pass over each file, so no rule can rewrite another
rule's output. That matters here: in this repo the console-script name IS the package name (see
``[project.scripts]`` in ``pyproject.toml``, where ``meeting_capture`` is both), so a sequential
search and replace would rename the command twice. The CLI rules therefore match only the places
a command NAME appears (the ``[project.scripts]`` key, and a documented shell invocation followed
by a subcommand or a flag), and the package rule takes every other occurrence.

``--dist`` defaults to the package name with underscores turned into hyphens. Note that this
repo's own distribution / git id is ``meeting-knowledge-capture`` while its package is
``meeting_capture``, so the two do NOT track each other here: pass ``--dist`` explicitly if your
git id differs from the hyphenated package name.
...
retention packs, fixtures, eval golden set) that this script deliberately does NOT make for you.
"""
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "meeting_capture"
_OLD_CLI = "meeting_capture"
_OLD_ENV_STEM = "MEETCAP"
_OLD_RESOURCE = "h6-svc"
_OLD_DIST = "meeting-knowledge-capture"
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        #      is a python module path and not a command however much it looks like one. This
        #      repo really does have one (`from meeting_capture import (` in
        #      scripts/portability_demo.py), and the package name is short enough that a fork
        #      should keep both guards rather than trusting the current tree.
        (
            "cli",
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both MEETCAP_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-meetings")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-meetings"
    )
    parser.add_argument(
        "--dist",
        default="",
        help=(
            "new distribution / git id (default: the package, hyphenated). This repo's own id "
            "does not follow that rule, so pass it when your git id differs from that"
        ),
    )
    ...
    print("Then work through docs/ADOPTING.md: region, IdP, retention packs, fixtures, eval set.")
    return 0
