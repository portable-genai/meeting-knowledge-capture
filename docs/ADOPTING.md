...
|---|---|---|
| `human-review-console` human-review and maker-checker console | every `requires_human_review` escalation in the catalog | **Wired.** `ports/review_router.py` with an adapter in all three families; the managed one submits over the shared `review-kit` to `review_url` (`HUMAN_REVIEW_URL`) and the service REFUSES TO BOOT with routing on and it empty; every caller reports `review_routing`, so a failed hand-off is visible rather than swallowed (`MEETCAP_REVIEW_ROUTING` switches routing). Rule R8 is Covered in `COMPLIANCE.md`. |
